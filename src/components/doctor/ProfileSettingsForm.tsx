      ...
      <form onSubmit={onSubmit} className="space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-2">
            ...
            <Input name="name" defaultValue={profile.name} required className="h-12 rounded-xl" />
          </div>
          
          <div className="space-y-2">
            <label className="text-xs font-bold uppercase tracking-wider text-[#1E3A5F]/70">Gender</label>
            <select name="gender" defaultValue={profile.gender || ""} className="flex h-12 w-full rounded-xl border border-input bg-transparent px-3 py-1 text-sm shadow-sm transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring">
          ...
          <div className="space-y-2">
            ...
            <Input name="specialization" defaultValue={profile.specialization} required className="h-12 rounded-xl" />
          </div>

          <div className="space-y-2">
            <label className="text-xs font-bold uppercase tracking-wider text-[#1E3A5F]/70">Sub-Specializations (Comma Separated)</label>
            <Input name="subSpecializations" defaultValue={profile.subSpecializations?.join(", ")} placeholder="e.g., Pediatric Cardiology, Echo" className="h-12 rounded-xl" />
          </div>
        </div>

        <div className="space-y-2">
          <label className="text-xs font-bold uppercase tracking-wider text-[#1E3A5F]/70">Professional Biography</label>
          <Textarea name="bio" defaultValue={profile.bio || ""} placeholder="Describe your experience..." className="min-h-30 rounded-xl" />
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-6 border-t border-[#6FAEE7]/10">
          <div className="space-y-2">
            ...
            <Input name="hmoAccreditations" defaultValue={profile.hmoAccreditations?.join(", ")} placeholder="e.g., Maxicare, Intellicare, PhilHealth" className="h-12 rounded-xl" />
          </div>
          <div className="space-y-2">
            <label className="text-xs font-bold uppercase tracking-wider text-[#1E3A5F]/70">Contact Number</label>
            <Input name="phoneNumber" defaultValue={profile.phoneNumber || ""} placeholder="+63 912 345 6789" className="h-12 rounded-xl" />
          </div>
          <div className="space-y-2">
            <label className="text-xs font-bold uppercase tracking-wider text-[#1E3A5F]/70">Clinic Room No.</label>
            <Input name="roomNumber" defaultValue={profile.roomNumber || ""} placeholder="e.g., Room 402, St. Luke's" className="h-12 rounded-xl" />
          </div>
          <div className="space-y-2">
            <label className="text-xs font-bold uppercase tracking-wider text-[#1E3A5F]/70">Standard Clinic Hours</label>
            <Input name="clinicHours" defaultValue={profile.clinicHours || ""} placeholder="e.g., 9:00 AM - 4:00 PM" className="h-12 rounded-xl" />
          </div>
        </div>
        ...
        <div className="flex justify-end pt-8 border-t border-[#6FAEE7]/10">
          <Button type="submit" disabled={isLoading} className="h-14 px-10 rounded-xl bg-[#1E3A5F] hover:bg-[#1E3A5F]/90 text-white font-bold text-lg shadow-md w-full md:w-auto">
